...
function ProductCard({ item, onOpen, onKnowledge }: { item: Product; onOpen: () => void; onKnowledge?: (knowledge: string) => void }) {
  return <article className="product-card" onClick={onOpen}>
    <div className="product-cover" style={{ background: item.color }}><span>{item.grade}</span><BookOpen size={37}/><strong>{item.subject}</strong><small>教师整理 · 平台审核</small></div>
    <div className="product-body"><div className="product-tags">{onKnowledge?<button title={`筛选${item.subject}`} onClick={event=>{event.stopPropagation();onKnowledge(item.subject)}}>{item.subject}</button>:<span>{item.subject}</span>}<span>{item.lessons} 道题</span></div><h3>{item.title}</h3><div className="teacher-row"><span className="teacher-avatar">{item.teacher[0]}</span><div><strong>{item.teacher}</strong><small>{item.credential}</small></div></div><footer><span><Star size={13} fill="currentColor"/>{item.rating}</span><strong>¥{item.price}</strong></footer></div>
  </article>
}
...
export function App() {
  ...
  return <div className={`agent-app home-page ${view === 'products' ? 'library-view' : ''}`}><div className="aurora aurora-one"/><div className="aurora aurora-two"/><div className="dot-grid"/>
    ...
    {view === 'products' && <main className="content-page"><div className="library-signal"><i/><span>TEACHER CURATED</span><b>持续更新</b></div><div className="page-heading"><span>教研资源库</span><h1>真人老师精选题库</h1><p>由一线教师整理、讲解与审核，按年级和专题持续更新。</p></div><div className="library-toolbar"><div className="filter-row">{['全部','初中数学','高中数学','几何','函数'].map(filter => <button key={filter} className={productFilter===filter?'active':''} onClick={() => setProductFilter(filter)}>{filter}</button>)}{!['全部','初中数学','高中数学','几何','函数'].includes(productFilter)&&<button className="active" onClick={()=>setProductFilter('全部')}>{productFilter}</button>}</div><span>共 {visibleProducts.length} 个专题</span></div><section className="product-grid">{visibleProducts.map(item => <ProductCard key={item.id} item={item} onOpen={() => openProduct(item)} onKnowledge={setProductFilter}/>)}</section><div className="library-footnote"><span>内容均由认证教师上传</span><i/><span>平台教研团队审核</span><i/><span>支持时序板书回放</span></div></main>}
