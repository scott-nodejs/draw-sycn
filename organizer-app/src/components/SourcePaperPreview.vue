...
const props=defineProps<{paper:Paper;question?:Question;questions?:Question[];recognizing?:boolean;editable?:boolean;splittable?:boolean;splitDisabled?:boolean}>()
const emit=defineEmits<{(event:'update:regions',regions:NonNullable<Question['sourceRegions']>):void;(event:'recognize'):void;(event:'split-page',pageNumber:number):void}>()
...
function regionsForPage(page:number){return displayQuestions.value.flatMap(question=>(question.sourceRegions||[]).map((region,index)=>({region,index,question,editable:canEdit.value&&question.id===props.question?.id}))).filter(item=>item.region.pageNumber===page)}
 ...
 <div ref="viewport" class="paper-source-preview">
  ...
  <div v-else class="paper-pages">
   <div v-for="(url,pageIndex) in pageUrls" :key="pageIndex" class="paper-source-page" :class="{'page-pending':!renderedPages.has(pageIndex+1)}" :data-page="pageIndex+1">
    <div class="page-actions"><button v-if="splittable" :disabled="splitDisabled" title="沿中线拆成左右两页" @click="emit('split-page',pageIndex+1)"><Scissors/>拆成两页</button><span>第 {{pageIndex+1}} 页</span></div><img v-if="url" :src="url" :alt="`${paper.title} 第 ${pageIndex+1} 页`" :loading="regionsForPage(pageIndex+1).length?'eager':'lazy'" draggable="false" @load="pageRendered(pageIndex+1)"/>
    ...
    <div v-for="item in renderedPages.has(pageIndex+1)?regionsForPage(pageIndex+1):[]" :key="`${item.question.id}-${item.index}`" class="paper-question-region" :class="{'read-only':!item.editable}" :style="{left:`${item.region.x0/10}%`,top:`${item.region.y0/10}%`,width:`${(item.region.x1-item.region.x0)/10}%`,height:`${(item.region.y1-item.region.y0)/10}%`}" @pointerdown="item.editable&&startAdjust($event,item.index,'move')">
     <div v-if="item.editable" class="region-float-actions" @pointerdown.stop><span>第 {{item.question.number}} 题</span><em><Move/>拖动或缩放红框</em><button :disabled="recognizing" @click.stop="emit('recognize')"><ScanLine/>{{recognizing?'识别中':'重新识别'}}</button></div>
     <span v-else class="region-number">第 {{item.question.number}} 题</span>
     <i v-for="handle in item.editable?handles:[]" :key="handle" class="resize-handle" :data-handle="handle" @pointerdown="startAdjust($event,item.index,handle)"></i>
    </div>
   </div>
  </div>
 </div>
...
.paper-question-region.read-only{cursor:default;background:rgba(240,68,56,.06);pointer-events:none}.region-number{position:absolute;left:-3px;top:-27px;height:25px;padding:0 8px;border-radius:5px 5px 0 0;background:#f04438;color:#fff;display:flex;align-items:center;font:700 11px/1 Inter,sans-serif;line-height:normal}
</style>
