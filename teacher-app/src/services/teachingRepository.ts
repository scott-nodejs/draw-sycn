...
export interface TeachingRepository {
  ...
  listMyQuestionSets(signal?: AbortSignal): Promise<LearningProduct[]>
  listQuestionSetStore(signal?: AbortSignal): Promise<LearningProduct[]>
  createQuestionSetPurchase(productId: string, signal?: AbortSignal): Promise<{id:string;status:string}>
  ...
  listTeacherSyncRooms(signal?: AbortSignal): Promise<SyncRoom[]>
...
class HttpTeachingRepository implements TeachingRepository {
  ...
  listLearningProducts(signal?: AbortSignal) {
    return this.request<LearningProduct[]>('/learning-products', { signal }).then(items=>items.filter(item=>item.productType!=='试题集'))
  }
  listMyQuestionSets(signal?: AbortSignal) { return this.request<LearningProduct[]>('/teacher/question-set-marketplace/mine',{signal}) }
  listQuestionSetStore(signal?: AbortSignal) { return this.request<LearningProduct[]>('/teacher/question-set-marketplace',{signal}) }
  createQuestionSetPurchase(productId:string,signal?:AbortSignal){return this.request<{id:string;status:string}>(`/teacher/question-set-marketplace/${encodeURIComponent(productId)}/purchases`,{method:'POST',signal})}
  ...
  listTeacherSyncRooms(signal?: AbortSignal) { return this.request<SyncRoom[]>('/teacher/sync-rooms', { signal }) }
  ...
  private async request<T>(path: string, init?: RequestInit): Promise<T> {
    ...
    const userRole = getStoredSession()?.user.role ?? 'teacher'
...
class DevelopmentTeachingRepository implements TeachingRepository {
  ...
  async listTeacherSyncRooms() { return [] }
  ...
  async listMyQuestionSets(){return []}
  async listQuestionSetStore(){return []}
  async createQuestionSetPurchase(){return {id:crypto.randomUUID(),status:'pending'}}
