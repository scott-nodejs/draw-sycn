...
function QuestionSetCenter({onBack}:{onBack:()=>void}) {
  ...
  return <main className="page-content question-set-page"><button className="back-link" onClick={onBack}><ArrowLeft size={16}/>返回试题库</button><div className="question-set-heading"><PageHeader eyebrow="QUESTION SETS" title="试题集" description="购买整理完成的高质量试题，减少重复切题、校对和结构化整理工作。"/><div className="question-set-assurance"><Check size={15}/><span>人工校对</span><i/><span>结构化题目</span><i/><span>购买后永久使用</span></div></div><div className="question-set-tabs"><button className={tab==="mine"?"active":""} onClick={()=>setTab("mine")}><span className="question-set-tab-icon"><BookOpenCheck size={18}/></span><span><strong>我的试题集</strong><small>已创建和已购买</small></span><b>{mine.length}</b></button><button className={tab==="store"?"active":""} onClick={()=>setTab("store")}><span className="question-set-tab-icon"><ShoppingBag size={18}/></span><span><strong>试题集商店</strong><small>发现优质教学内容</small></span><b>{storeItems.length}</b></button></div>{message?<div className="invite-code question-set-message"><span>{message}</span></div>:null}{loading?<LoadingState/>:<section className="question-set-grid">{items.length?items.map(item=><article className="content-card question-set-card" key={item.id}><div className={`product-cover-preview ${item.coverStyle}`}><span>{item.subject} · {item.grade}</span><BookOpenCheck size={30}/><strong>{item.title}</strong><small>{item.teacherName||"专业整理团队"}</small></div><div className="question-set-card-body"><div className="question-set-copy"><span className="eyebrow">已校对 · 可直接导入</span><h2>{item.title}</h2><p>{item.subtitle||item.description||"已完成题目切分、识别与人工校对"}</p></div><div className="question-set-purchase"><div className="question-set-meta"><span><b>{item.questionIds.length||item.lessonCount}</b> 道精选题</span><strong><small>¥</small>{item.price.toFixed(2)}</strong></div>{tab==="store"?<button className="button primary" onClick={()=>void teachingRepository.createQuestionSetPurchase(item.id).then(order=>setMessage(order.status==='pending'?"订单已创建，支付成功后将自动加入我的试题集":"试题集已加入我的内容")).catch(cause=>setMessage(cause instanceof Error?cause.message:"创建订单失败"))}><ShoppingBag size={16}/>购买试题集</button>:<button className="button secondary"><BookOpenCheck size={16}/>查看与导入</button>}</div></div></article>):<EmptyCollection message={tab==="mine"?"还没有试题集。购买成功或由整理端创建后会出现在这里。":"商店暂时没有已上架的试题集。"}/>}</section>}</main>;
}
