...
server.on('upgrade', async (request, socket, head) => {
  ...
  if (process.env.SYNC_REQUIRE_AUTH !== 'false') {
    ...
    try{
      ...
      const response=await fetch(`${apiBase}/classroom/rooms/${encodeURIComponent(roomId)}/canvas/access`,{headers:{...authHeaders,'X-User-Id':String(user.id),'X-User-Role':String(user.role)}})
