...
export default function LiveRoom() {
  ...
  const { id = '' } = useParams()
  const { rooms,user } = useStudentApi()
  ...
  useEffect(()=>{if(!id)return;return connectClassroomSocket(id,event=>{if(event.targetUserId&&event.targetUserId!==user?.id)return;if(event.event==='RTC_INVITE')setInvited(true);if(event.event==='RTC_KICK'||event.event==='ROOM_ENDED'){void rtcRef.current?.stopMicrophone().catch(()=>undefined);setConnected(false);setInvited(false)}if(event.event==='RTC_MUTED'){void rtcRef.current?.mute().catch(()=>undefined);setMuted(true)}if(event.event==='RTC_UNMUTED'){void rtcRef.current?.unmute().catch(()=>undefined);setMuted(false)}if(event.event==='CANVAS_PERMISSION_GRANTED')setCanWrite(true);if(event.event==='CANVAS_PERMISSION_REVOKED')setCanWrite(false)})},[id,user?.id])
