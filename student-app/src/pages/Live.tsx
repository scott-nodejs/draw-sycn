        ...
        <aside className="card h-fit p-4">
          ...
          <div className="mt-4 space-y-5">
            <div>
              ...
              {['全部班级',...classes.map(item=>item.name)].map((x,i)=><button key={x} className={`mb-1 w-full rounded-xl px-3 py-2 text-left text-sm ${i===0?'bg-blue-50 text-blue-700':'text-slate-600 hover:bg-slate-50'}`}>{x}</button>)}
            </div>
            <div>
              <div className="mb-2 text-xs font-medium text-slate-400">按老师</div>
              {[...new Set(rooms.map(item=>item.teacher))].map(x=><button key={x} className="mb-1 w-full rounded-xl px-3 py-2 text-left text-sm text-slate-600 hover:bg-slate-50">{x}</button>)}
            </div>
          </div>
        </aside>
        ...
        <section className="space-y-4">
          <div className="card flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between">
            <div className="flex gap-1.5">
              {['全部','进行中','未开始','已结束'].map((x,i)=><button key={x} className={`rounded-xl px-4 py-2 text-sm font-medium ${i===0?'bg-blue-600 text-white':'text-slate-600 hover:bg-slate-50'}`}>{x}</button>)}
            </div>
            <div className="relative">
              <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/>
              <input className="h-10 rounded-xl border border-slate-200 pl-9 pr-3 text-sm outline-none focus:border-blue-300" placeholder="搜索房间"/>
            </div>
          </div>
            ...
            <div key={room.id} className="card p-5">
              <div className="grid gap-4 md:grid-cols-[1fr_auto] md:items-center">
                ...
                <div>
                  ...
                  {room.status==='已结束' && <button className="btn-secondary text-slate-300" disabled><PlayCircle size={16}/>查看回放</button>}
                </div>
              </div>
            </div>
          ))}
