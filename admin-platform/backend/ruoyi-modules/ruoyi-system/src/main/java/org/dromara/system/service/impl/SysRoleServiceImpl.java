...
import org.dromara.common.core.constant.AppConstants;
...
public class SysRoleServiceImpl implements ISysRoleService, RoleService {
    ...
    private Wrapper<SysRole> buildQueryWrapper(SysRoleBo bo) {
        ...
        wrapper.eq(ObjectUtil.isNotNull(bo.getRoleId()), SysRole::getRoleId, bo.getRoleId())
            .eq(SysRole::getAppId, ObjectUtil.defaultIfNull(bo.getAppId(), AppConstants.ADMIN_ID))
    ...
    /**
     * 角色只能绑定同一应用下的菜单；租户角色还必须落在平台授予该租户的范围内。
     */
    private void validateRoleMenus(SysRoleBo role) {
        if (role.getAppId() == null) {
            throw new ServiceException("角色所属应用不能为空");
        }
        if (role.getMenuIds() == null || role.getMenuIds().length == 0) {
            return;
        }
        Set<Long> menuIds = new HashSet<>(List.of(role.getMenuIds()));
        long appMenuCount = menuMapper.selectCount(new LambdaQueryWrapper<SysMenu>()
            .eq(SysMenu::getAppId, role.getAppId())
            .eq(SysMenu::getStatus, SystemConstants.NORMAL)
            .in(SysMenu::getMenuId, menuIds));
        if (appMenuCount != menuIds.size()) {
            throw new ServiceException("角色菜单包含其他应用或已停用的菜单");
        }
        if (LoginHelper.isSuperAdmin()) {
            return;
        }
        long tenantMenuCount = tenantMenuMapper.selectCount(new LambdaQueryWrapper<SysTenantMenu>()
            .eq(SysTenantMenu::getTenantId, LoginHelper.getTenantId())
            .eq(SysTenantMenu::getAppId, role.getAppId())
            .in(SysTenantMenu::getMenuId, menuIds)
            .and(w -> w.isNull(SysTenantMenu::getExpireTime)
                .or().gt(SysTenantMenu::getExpireTime, LocalDateTime.now())));
        if (tenantMenuCount != menuIds.size()) {
            throw new ServiceException("角色菜单超出租户已授权功能范围");
        }
    }
